...
cleanup() {
    ...
    if type onExit | grep -q "shell function" ; then
        onExit					# Run script-specific cleanup function
    fi
}
temp=$(mktemp)
...
run() {
	...
	if [[ $code = 0 ]] ; then
		...
		if type onFail | grep -q "shell function" ; then
			onFail
		fi
		[[ $NOTIFY_ON_FAIL = 1 ]] && notify-send -u critical \
			-c TYPE "$(basename "$0")" \
			"The script failed to complete"
		[[ $SLEEP_ON_FAIL = 1 ]] && sleep infinity || exit 2
	fi
}
